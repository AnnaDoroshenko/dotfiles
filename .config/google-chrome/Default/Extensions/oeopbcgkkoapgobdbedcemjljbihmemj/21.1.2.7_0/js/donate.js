...
function initPaymentDetails(buttonClicked) {
	...
	if (ITEM_ID == "screenshots" && !email && (isMonthly() || buttonClicked == "alipay")) {
		let promptMessage;
		if (isMonthly()) {
	...
	if (donateButtonClicked == "paypal") {
		...
		*/
		...
		let data = {
			action: "createSource",
			type: "alipay",
			amount: getStripeAmount(price, getCurrencyCode()),
			currency: getCurrencyCode(),
			email: email,
			itemId: ITEM_ID,
			itemName: extensionName,
			description: extensionName,
			extensionUrl: donationPageUrl + "?action=alipay",
		...
		ajax({
			...
			sendGA("alipay", 'error with token: ' + jqXHR.responseText);
		});
		...
		var $dialog = initTemplate("applePayDialogTemplate");
		$dialog.find(".dialogDescription").html("Scan this QR code with your iOS device.<br><br><img src='https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=" + encodeURIComponent(url) + "'/>");
		$dialog.find(".continue").off().on("click", function () {
			...
			Controller.verifyPayment(ITEM_ID, email).then(response => {
				...
				if (response.unlocked) {
					...
					openGenericDialog({
						content: "You must scan the code and complete the payment first.<br><br>If you did this then <a href='https://jasonsavard.com/contact'>contact</a> the developer!"
					});
				}
			}).catch(error => {
				$dialog[0].close();
				...
				showSuccessfulPayment();
				sendGA("applePay", 'failure ' + error + ' but show success anyways');
			});
		})
		openDialog($dialog);
