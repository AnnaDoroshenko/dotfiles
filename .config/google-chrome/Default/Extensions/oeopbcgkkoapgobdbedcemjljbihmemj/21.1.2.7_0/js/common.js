...
function OAuthForDevices(params) {
	...
	function initError(jqXHR, tokenResponse, oauthAction) {
		...
		if (error == "invalid_grant" || error.code == 400 || error.code == 401) {
	...
	function ensureToken(tokenResponse) {
		return new Promise((resolve, reject) => {
			if (tokenResponse.chromeProfile) {
				let getAuthTokenParams = { interactive: false, scopes: that.oauthParams.scope.split(" ") };
	...
	this.removeAllCachedTokens = function() {
		let removeTokenPromises = [];
		that.oauthParams.tokenResponses.forEach(tokenResponse => {
			removeTokenPromises = removeCachedAuthToken(tokenResponse.access_token);
		});
		...
		return new Promise((resolve, reject) => {
			if (params.code) {
				...
				new Promise((resolve, reject) => {
					if (params.refetch) {
						if (params.userEmail) {
							let tokenResponse = that.findTokenResponse({ userEmail: userEmail });
							...
							that.removeAllCachedTokens().then(() => {
								resolve();
							});
						}
					} else {
					...
					let tokenResponse = { expires_in: 9999999999, chromeProfile: true };
					let getAuthTokenParams = { interactive: true, scopes: that.oauthParams.scope.split(" ") };
					chrome.identity.getAuthToken(getAuthTokenParams, token => {
						if (chrome.runtime.lastError) {
...
function keydown(e, key, comboKeys) {
	if (e) {
		...
		if (keyMatched) {
			...
			comboKeysMatched = comboKeys.ctrl == isCtrlPressed(e) && comboKeys.shift == e.shiftKey;
		}
