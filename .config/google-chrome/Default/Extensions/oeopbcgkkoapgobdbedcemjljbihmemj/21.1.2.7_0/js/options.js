...
function firefoxPatchForCheckbox() {
...
function initPage(tabName) {
	...
	if (!$("#" + tabName + "Page").length) {
		...
		if (tabName == "welcome") {
			...
			if (Settings.read("dndDuringCalendarEvent")) {
			...
			$("#dndDuringCalendarEvent").change(function() {
				...
				sendMessageToCalendarExtension({action:"getInfo"}).then(response => {
					if (response) {
						Settings.store("dndDuringCalendarEvent", checked);
					} else {
			...
			$("#pollingInterval paper-listbox").on("iron-activate", function(e) {
			...
			$("#addAccount").click(function () {
				let tokenResponses = Settings.read("tokenResponsesEmails");
				...
				if ((tokenResponses && tokenResponses.length) || DetectClient.isFirefox()) {
					requestPermission({ oAuthForDevices: bg.oAuthForEmails, useGoogleAccountsSignIn: true });
				} else {
					openPermissionsDialog({ oAuthForDevices: bg.oAuthForEmails }).then(response => {
...
function loadAccountsOptions(loadAccountsParams) {
	...
	if (allAccounts.length || Settings.read("accountAddingMethod") == "oauth") {
		jqueryEmpty($monitorLabels);
	}
	...
	setTimeout(function() {
		...
		if (Settings.read("accountAddingMethod") == "autoDetect" && (accountsList.length == 0 || (lastError && lastErrorCode != JError.CANNOT_ENSURE_MAIN_AND_INBOX_UNREAD))) {
...
function initDisplayForAccountAddingMethod() {
	...
	if ($pollingInterval.length) {
		$pollingInterval[0].select(calculatePollingInterval(bg.accounts));
	}
}

function grantPermissionToEmails(tokenResponse) {
	showLoading();
	...
	if (!account) {
		...
		account = new bg.MailAccount({ mailAddress: tokenResponse.userEmail });
	...
	polymerPromise.then(function() {
		...
		chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
			if (message.command == "grantPermissionToEmails") {
				grantPermissionToEmails(message.tokenResponse);
				sendResponse();
			}
		});
