...
function MailAccount(accountParams) {
   ...
   this.hasHiddenTabs = function() {
		var gmailSettings = that.getSetting("gmailSettings");
		return that.isUsingGmailCategories() && (gmailSettings.tabs && gmailSettings.tabs.length < TOTAL_GMAIL_TABS); // 5 means ok because they are all shown
   }
   ...
   this.getMonitorLabels = function() {
	   ...
	   /*
	   let monitorLabels;
	   if (ltVersion(Settings.read("installVersion"), "21.0")) { // originally: 19.2.17
		   monitorLabels = that.getSetting("monitorLabel", "monitorLabelsForGmailClassic");
	   } else {
		   if (that.isUsingGmailCategories()) {
			   monitorLabels = that.getSetting("monitorLabel");
			   if (!monitorLabels) {
				   monitorLabels = [SYSTEM_PRIMARY];
				   // if tabs are hidden then we must monitor them because Gmail merges then into the Primary tab (even though they don't have the primary label)
				   let gmailSettings = that.getSetting("gmailSettings");
				   if (gmailSettings.tabs) {
					   if (gmailSettings.tabs.length < TOTAL_GMAIL_TABS) {
						   if (gmailSettings.tabs.indexOf(SYSTEM_SOCIAL) == -1) {
							   monitorLabels.push(SYSTEM_SOCIAL);
						   }
						   if (gmailSettings.tabs.indexOf(SYSTEM_PROMOTIONS) == -1) {
							   monitorLabels.push(SYSTEM_PROMOTIONS);
						   }
						   if (gmailSettings.tabs.indexOf(SYSTEM_UPDATES) == -1) {
							   monitorLabels.push(SYSTEM_UPDATES);
						   }
						   if (gmailSettings.tabs.indexOf(SYSTEM_FORUMS) == -1) {
							   monitorLabels.push(SYSTEM_FORUMS);
						   }
					   } else { // avoid polling all labels and just monitor inbox instead
							monitorLabels = [SYSTEM_INBOX];
					   }
				   }
			   }
		   } else {
   ...
   this.remove = function(oAuthForEmails, accounts) {
	   if (Settings.read("accountAddingMethod") == "autoDetect") {
		   ...
		   let tokenResponse = oAuthForEmails.findTokenResponse({ userEmail: that.getAddress() });
		   if (tokenResponse) {
