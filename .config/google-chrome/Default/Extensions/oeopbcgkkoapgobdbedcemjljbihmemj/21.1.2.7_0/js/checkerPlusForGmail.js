...
function updateBadge(totalUnread) {
	...
	getDNDState().then(dndState => {
		...
		try {
			chrome.contextMenus.update(window.markAllAsReadMenuId, {visible:(totalUnread >= 1)});
		} catch (error) {
...
function generateContactPhotoURL(contact, account) {
	return new Promise((resolve, reject) => {
		if (contact.photoUrl) {
			bg.oAuthForContacts.generateURL(account.getAddress(), contact.photoUrl).then(function (response) {
...
function isEligibleForReducedDonation(mightBeShown) {
	...
	if (Settings.read("donationClicked") || !Settings.read("verifyPaymentRequestSentForReducedDonation")) {
		...
		if (daysElapsedSinceFirstInstalled() >= UserNoticeSchedule.DAYS_BEFORE_ELIGIBLE_FOR_REDUCED_DONATION) { // 14 days
...
function shouldShowExtraFeature() {
	...
	if (Settings.read("donationClicked") || !Settings.read("verifyPaymentRequestSentForReducedDonation")) {
		...
		if (skins && skins.length) {
			...
			if (daysElapsedSinceFirstInstalled() >= UserNoticeSchedule.DAYS_BEFORE_SHOWING_EXTRA_FEATURE) {
...
function requestPermission(params) {
	return new Promise((resolve, reject) => {
		...
		if (params.useGoogleAccountsSignIn) {
			...
			if (window.inWidget) {
				window.permissionGrantedInterval = setInterval(function () {
					if (Settings.read("tokenResponsesEmails")) {
						clearInterval(window.permissionGrantedInterval);
						resolve();
					}
				}, 2000);
			}
...
function syncSignInOrderForAllAccounts() {
	...
	return Promise.all(promises).then((promiseAllResponse) => {
