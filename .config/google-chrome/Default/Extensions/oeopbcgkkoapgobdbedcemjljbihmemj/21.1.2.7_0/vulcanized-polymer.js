...
*/
goog.provide("module$src$template_map");
var templateMap$$module$src$template_map = {};
var $jscompDefaultExport$$module$src$template_map = templateMap$$module$src$template_map;
module$src$template_map.default = $jscompDefaultExport$$module$src$template_map;
goog.provide("module$src$css_parse");
var StyleNode$$module$src$css_parse = function() {
  this["start"] = 0;
  this["end"] = 0;
  this["previous"] = null;
  this["parent"] = null;
...
function parse$$module$src$css_parse(text) {
  text = clean$$module$src$css_parse(text);
  return parseCss$$module$src$css_parse(lex$$module$src$css_parse(text), text);
}
function clean$$module$src$css_parse(cssText) {
  return cssText.replace(RX$$module$src$css_parse.comments, "").replace(RX$$module$src$css_parse.port, "");
}
function lex$$module$src$css_parse(text) {
  var root = new StyleNode$$module$src$css_parse;
  root["start"] = 0;
  root["end"] = text.length;
  var n = root;
  for (var i = 0, l = text.length; i < l; i++) {
    if (text[i] === OPEN_BRACE$$module$src$css_parse) {
      if (!n["rules"]) {
        n["rules"] = [];
      }
      var p = n;
      var previous = p["rules"][p["rules"].length - 1] || null;
      n = new StyleNode$$module$src$css_parse;
      n["start"] = i + 1;
      n["parent"] = p;
      n["previous"] = previous;
      p["rules"].push(n);
    } else {
      if (text[i] === CLOSE_BRACE$$module$src$css_parse) {
        n["end"] = i + 1;
        n = n["parent"] || root;
      }
    }
...
function parseCss$$module$src$css_parse(node, text) {
  var t = text.substring(node["start"], node["end"] - 1);
  node["parsedCssText"] = node["cssText"] = t.trim();
  if (node["parent"]) {
    var ss = node["previous"] ? node["previous"]["end"] : node["parent"]["start"];
    t = text.substring(ss, node["start"] - 1);
    t = _expandUnicodeEscapes$$module$src$css_parse(t);
    t = t.replace(RX$$module$src$css_parse.multipleSpaces, " ");
    t = t.substring(t.lastIndexOf(";") + 1);
    var s = node["parsedSelector"] = node["selector"] = t.trim();
    node["atRule"] = s.indexOf(AT_START$$module$src$css_parse) === 0;
    if (node["atRule"]) {
      if (s.indexOf(MEDIA_START$$module$src$css_parse) === 0) {
        node["type"] = types$$module$src$css_parse.MEDIA_RULE;
      } else {
        if (s.match(RX$$module$src$css_parse.keyframesRule)) {
          node["type"] = types$$module$src$css_parse.KEYFRAMES_RULE;
          node["keyframesName"] = node["selector"].split(RX$$module$src$css_parse.multipleSpaces).pop();
        }
      }
    } else {
      if (s.indexOf(VAR_START$$module$src$css_parse) === 0) {
        node["type"] = types$$module$src$css_parse.MIXIN_RULE;
      } else {
        node["type"] = types$$module$src$css_parse.STYLE_RULE;
      }
    }
  }
  var r$ = node["rules"];
  if (r$) {
    for (var i = 0, l = r$.length, r; i < l && (r = r$[i]); i++) {
      parseCss$$module$src$css_parse(r, text);
    }
  }
  return node;
}
function _expandUnicodeEscapes$$module$src$css_parse(s) {
...
function stringify$$module$src$css_parse(node, preserveProperties, text) {
  text = text === undefined ? "" : text;
  var cssText = "";
  if (node["cssText"] || node["rules"]) {
    var r$ = node["rules"];
    if (r$ && !_hasMixinRules$$module$src$css_parse(r$)) {
      for (var i = 0, l = r$.length, r; i < l && (r = r$[i]); i++) {
        cssText = stringify$$module$src$css_parse(r, preserveProperties, cssText);
      }
    } else {
      cssText = preserveProperties ? node["cssText"] : removeCustomProps$$module$src$css_parse(node["cssText"]);
  ...
  if (cssText) {
    if (node["selector"]) {
      text += node["selector"] + " " + OPEN_BRACE$$module$src$css_parse + "\n";
    }
    text += cssText;
    if (node["selector"]) {
      text += CLOSE_BRACE$$module$src$css_parse + "\n\n";
    }
  }
  return text;
}
function _hasMixinRules$$module$src$css_parse(rules) {
  var r = rules[0];
  return Boolean(r) && Boolean(r["selector"]) && r["selector"].indexOf(VAR_START$$module$src$css_parse) === 0;
}
function removeCustomProps$$module$src$css_parse(cssText) {
  cssText = removeCustomPropAssignment$$module$src$css_parse(cssText);
  return removeCustomPropApply$$module$src$css_parse(cssText);
}
function removeCustomPropAssignment$$module$src$css_parse(cssText) {
  return cssText.replace(RX$$module$src$css_parse.customProp, "").replace(RX$$module$src$css_parse.mixinProp, "");
}
function removeCustomPropApply$$module$src$css_parse(cssText) {
  return cssText.replace(RX$$module$src$css_parse.mixinApply, "").replace(RX$$module$src$css_parse.varApply, "");
}
var types$$module$src$css_parse = {STYLE_RULE:1, KEYFRAMES_RULE:7, MEDIA_RULE:4, MIXIN_RULE:1000};
var OPEN_BRACE$$module$src$css_parse = "{";
var CLOSE_BRACE$$module$src$css_parse = "}";
var RX$$module$src$css_parse = {comments:/\/\*[^*]*\*+([^/*][^*]*\*+)*\//gim, port:/@import[^;]*;/gim, customProp:/(?:^[^;\-\s}]+)?--[^;{}]*?:[^{};]*?(?:[;\n]|$)/gim, mixinProp:/(?:^[^;\-\s}]+)?--[^;{}]*?:[^{};]*?{[^}]*?}(?:[;\n]|$)?/gim, mixinApply:/@apply\s*\(?[^);]*\)?\s*(?:[;\n]|$)?/gim, varApply:/[^;:]*?:[^;]*?var\([^;]*\)(?:[;\n]|$)?/gim, keyframesRule:/^@[^\s]*keyframes/, multipleSpaces:/\s+/g};
var VAR_START$$module$src$css_parse = "--";
var MEDIA_START$$module$src$css_parse = "@media";
var AT_START$$module$src$css_parse = "@";
module$src$css_parse.StyleNode = StyleNode$$module$src$css_parse;
...
goog.provide("module$src$apply_shim_utils");
goog.require("module$src$template_map");
goog.require("module$src$css_parse");
var CURRENT_VERSION$$module$src$apply_shim_utils = "_applyShimCurrentVersion";
var NEXT_VERSION$$module$src$apply_shim_utils = "_applyShimNextVersion";
var VALIDATING_VERSION$$module$src$apply_shim_utils = "_applyShimValidatingVersion";
var promise$$module$src$apply_shim_utils = Promise.resolve();
function invalidate$$module$src$apply_shim_utils(elementName) {
  var template = module$src$template_map.default[elementName];
  if (template) {
    invalidateTemplate$$module$src$apply_shim_utils(template);
  }
}
function invalidateTemplate$$module$src$apply_shim_utils(template) {
  template[CURRENT_VERSION$$module$src$apply_shim_utils] = template[CURRENT_VERSION$$module$src$apply_shim_utils] || 0;
  template[VALIDATING_VERSION$$module$src$apply_shim_utils] = template[VALIDATING_VERSION$$module$src$apply_shim_utils] || 0;
  template[NEXT_VERSION$$module$src$apply_shim_utils] = (template[NEXT_VERSION$$module$src$apply_shim_utils] || 0) + 1;
}
function isValid$$module$src$apply_shim_utils(elementName) {
  var template = module$src$template_map.default[elementName];
  if (template) {
    return templateIsValid$$module$src$apply_shim_utils(template);
  }
  return true;
}
function templateIsValid$$module$src$apply_shim_utils(template) {
  return template[CURRENT_VERSION$$module$src$apply_shim_utils] === template[NEXT_VERSION$$module$src$apply_shim_utils];
}
function isValidating$$module$src$apply_shim_utils(elementName) {
  var template = module$src$template_map.default[elementName];
  if (template) {
    return templateIsValidating$$module$src$apply_shim_utils(template);
  }
  return false;
}
function templateIsValidating$$module$src$apply_shim_utils(template) {
  return !templateIsValid$$module$src$apply_shim_utils(template) && template[VALIDATING_VERSION$$module$src$apply_shim_utils] === template[NEXT_VERSION$$module$src$apply_shim_utils];
}
function startValidating$$module$src$apply_shim_utils(elementName) {
  var template = module$src$template_map.default[elementName];
  startValidatingTemplate$$module$src$apply_shim_utils(template);
}
function startValidatingTemplate$$module$src$apply_shim_utils(template) {
  template[VALIDATING_VERSION$$module$src$apply_shim_utils] = template[NEXT_VERSION$$module$src$apply_shim_utils];
  if (!template._validating) {
    template._validating = true;
    promise$$module$src$apply_shim_utils.then(function() {
      template[CURRENT_VERSION$$module$src$apply_shim_utils] = template[NEXT_VERSION$$module$src$apply_shim_utils];
      template._validating = false;
    });
  }
}
function elementsAreInvalid$$module$src$apply_shim_utils() {
  for (var elementName in module$src$template_map.default) {
    var template = module$src$template_map.default[elementName];
    if (!templateIsValid$$module$src$apply_shim_utils(template)) {
      return true;
    }
  }
  return false;
}
module$src$apply_shim_utils.invalidate = invalidate$$module$src$apply_shim_utils;
module$src$apply_shim_utils.invalidateTemplate = invalidateTemplate$$module$src$apply_shim_utils;
module$src$apply_shim_utils.isValid = isValid$$module$src$apply_shim_utils;
module$src$apply_shim_utils.templateIsValid = templateIsValid$$module$src$apply_shim_utils;
module$src$apply_shim_utils.isValidating = isValidating$$module$src$apply_shim_utils;
module$src$apply_shim_utils.templateIsValidating = templateIsValidating$$module$src$apply_shim_utils;
module$src$apply_shim_utils.startValidating = startValidating$$module$src$apply_shim_utils;
module$src$apply_shim_utils.startValidatingTemplate = startValidatingTemplate$$module$src$apply_shim_utils;
module$src$apply_shim_utils.elementsAreInvalid = elementsAreInvalid$$module$src$apply_shim_utils;
goog.provide("module$src$common_regex");
var VAR_ASSIGN$$module$src$common_regex = /(?:^|[;\s{]\s*)(--[\w-]*?)\s*:\s*(?:((?:'(?:\\'|.)*?'|"(?:\\"|.)*?"|\([^\)]*?\)|[^};{])+)|\{([^\}]*)\}(?:(?=[;\s}])|$))/gi;
var MIXIN_MATCH$$module$src$common_regex = /(?:^|\W+)@apply\s*\(?([^);\n]*)\)?/gi;
var VAR_CONSUMED$$module$src$common_regex = /(--[\w-]+)\s*([:,;)]|$)/gi;
var ANIMATION_MATCH$$module$src$common_regex = /(animation\s*:)|(animation-name\s*:)/;
var MEDIA_MATCH$$module$src$common_regex = /@media[^(]*(\([^)]*\))/;
var IS_VAR$$module$src$common_regex = /^--/;
var BRACKETED$$module$src$common_regex = /\{[^}]*\}/g;
var HOST_PREFIX$$module$src$common_regex = "(?:^|[^.#[:])";
var HOST_SUFFIX$$module$src$common_regex = "($|[.:[\\s>+~])";
module$src$common_regex.VAR_ASSIGN = VAR_ASSIGN$$module$src$common_regex;
module$src$common_regex.MIXIN_MATCH = MIXIN_MATCH$$module$src$common_regex;
module$src$common_regex.VAR_CONSUMED = VAR_CONSUMED$$module$src$common_regex;
module$src$common_regex.ANIMATION_MATCH = ANIMATION_MATCH$$module$src$common_regex;
module$src$common_regex.MEDIA_MATCH = MEDIA_MATCH$$module$src$common_regex;
module$src$common_regex.IS_VAR = IS_VAR$$module$src$common_regex;
module$src$common_regex.BRACKETED = BRACKETED$$module$src$common_regex;
module$src$common_regex.HOST_PREFIX = HOST_PREFIX$$module$src$common_regex;
module$src$common_regex.HOST_SUFFIX = HOST_SUFFIX$$module$src$common_regex;
goog.provide("module$src$style_settings");
var nativeShadow$$module$src$style_settings = !(window["ShadyDOM"] && window["ShadyDOM"]["inUse"]);
var nativeCssVariables$$module$src$style_settings;
function calcCssVariables$$module$src$style_settings(settings) {
  if (settings && settings["shimcssproperties"]) {
    nativeCssVariables$$module$src$style_settings = false;
  } else {
    nativeCssVariables$$module$src$style_settings = nativeShadow$$module$src$style_settings || Boolean(!navigator.userAgent.match("AppleWebKit/601") && window.CSS && CSS.supports && CSS.supports("box-shadow", "0 0 0 var(--foo)"));
  }
}
if (window.ShadyCSS && window.ShadyCSS.nativeCss !== undefined) {
  nativeCssVariables$$module$src$style_settings = window.ShadyCSS.nativeCss;
} else {
  if (window.ShadyCSS) {
    calcCssVariables$$module$src$style_settings(window.ShadyCSS);
    window.ShadyCSS = undefined;
  } else {
    calcCssVariables$$module$src$style_settings(window["WebComponents"] && window["WebComponents"]["flags"]);
  }
}
...
goog.provide("module$src$style_util");
goog.require("module$src$style_settings");
goog.require("module$src$css_parse");
goog.require("module$src$common_regex");
function toCssText$$module$src$style_util(rules, callback) {
  if (!rules) {
    return "";
  }
  if (typeof rules === "string") {
    rules = module$src$css_parse.parse(rules);
  }
  if (callback) {
    forEachRule$$module$src$style_util(rules, callback);
  }
  return module$src$css_parse.stringify(rules, module$src$style_settings.nativeCssVariables);
}
function rulesForStyle$$module$src$style_util(style) {
  if (!style["__cssRules"] && style.textContent) {
    style["__cssRules"] = module$src$css_parse.parse(style.textContent);
  }
  return style["__cssRules"] || null;
}
function isKeyframesSelector$$module$src$style_util(rule) {
  return Boolean(rule["parent"]) && rule["parent"]["type"] === module$src$css_parse.types.KEYFRAMES_RULE;
}
function forEachRule$$module$src$style_util(node, styleRuleCallback, keyframesRuleCallback, onlyActiveRules) {
  if (!node) {
    return;
  }
  var skipRules = false;
  var type = node["type"];
  if (onlyActiveRules) {
    if (type === module$src$css_parse.types.MEDIA_RULE) {
      var matchMedia = node["selector"].match(module$src$common_regex.MEDIA_MATCH);
      if (matchMedia) {
  ...
  if (type === module$src$css_parse.types.STYLE_RULE) {
    styleRuleCallback(node);
  } else {
    if (keyframesRuleCallback && type === module$src$css_parse.types.KEYFRAMES_RULE) {
      keyframesRuleCallback(node);
    } else {
      if (type === module$src$css_parse.types.MIXIN_RULE) {
        skipRules = true;
      }
    }
  }
  ...
  if (r$ && !skipRules) {
    for (var i = 0, l = r$.length, r; i < l && (r = r$[i]); i++) {
      forEachRule$$module$src$style_util(r, styleRuleCallback, keyframesRuleCallback, onlyActiveRules);
    }
  }
}
function applyCss$$module$src$style_util(cssText, moniker, target, contextNode) {
  var style = createScopeStyle$$module$src$style_util(cssText, moniker);
  applyStyle$$module$src$style_util(style, target, contextNode);
  return style;
}
function createScopeStyle$$module$src$style_util(cssText, moniker) {
...
function applyStylePlaceHolder$$module$src$style_util(moniker) {
  var placeHolder = document.createComment(" Shady DOM styles for " + moniker + " ");
  var after = lastHeadApplyNode$$module$src$style_util ? lastHeadApplyNode$$module$src$style_util["nextSibling"] : null;
  var scope = document.head;
  scope.insertBefore(placeHolder, after || scope.firstChild);
  lastHeadApplyNode$$module$src$style_util = placeHolder;
  return placeHolder;
}
function applyStyle$$module$src$style_util(style, target, contextNode) {
  target = target || document.head;
  var after = contextNode && contextNode.nextSibling || target.firstChild;
  target.insertBefore(style, after);
  if (!lastHeadApplyNode$$module$src$style_util) {
    lastHeadApplyNode$$module$src$style_util = style;
  } else {
    var position = style.compareDocumentPosition(lastHeadApplyNode$$module$src$style_util);
    if (position === Node.DOCUMENT_POSITION_PRECEDING) {
      lastHeadApplyNode$$module$src$style_util = style;
    }
  }
}
function isTargetedBuild$$module$src$style_util(buildType) {
  return module$src$style_settings.nativeShadow ? buildType === "shadow" : buildType === "shady";
}
function getCssBuildType$$module$src$style_util(element) {
  return element.getAttribute("css-build");
}
function findMatchingParen$$module$src$style_util(text, start) {
  var level = 0;
...
function processVariableAndFallback$$module$src$style_util(str, callback) {
  var start = str.indexOf("var(");
  if (start === -1) {
    return callback(str, "", "", "");
  }
  var end = findMatchingParen$$module$src$style_util(str, start + 3);
  var inner = str.substring(start + 4, end);
  var prefix = str.substring(0, start);
  var suffix = processVariableAndFallback$$module$src$style_util(str.substring(end + 1), callback);
...
function setElementClassRaw$$module$src$style_util(element, value) {
  if (module$src$style_settings.nativeShadow) {
    element.setAttribute("class", value);
  } else {
    window["ShadyDOM"]["nativeMethods"]["setAttribute"].call(element, "class", value);
  }
}
function getIsExtends$$module$src$style_util(element) {
...
module$src$style_util.setElementClassRaw = setElementClassRaw$$module$src$style_util;
module$src$style_util.getIsExtends = getIsExtends$$module$src$style_util;
goog.provide("module$src$common_utils");
goog.require("module$src$common_regex");
function updateNativeProperties$$module$src$common_utils(element, properties) {
...
function getComputedStyleValue$$module$src$common_utils(element, property) {
...
function detectMixin$$module$src$common_utils(cssText) {
  var has = module$src$common_regex.MIXIN_MATCH.test(cssText) || module$src$common_regex.VAR_ASSIGN.test(cssText);
  module$src$common_regex.MIXIN_MATCH.lastIndex = 0;
  module$src$common_regex.VAR_ASSIGN.lastIndex = 0;
  return has;
}
module$src$common_utils.updateNativeProperties = updateNativeProperties$$module$src$common_utils;
module$src$common_utils.getComputedStyleValue = getComputedStyleValue$$module$src$common_utils;
module$src$common_utils.detectMixin = detectMixin$$module$src$common_utils;
goog.provide("module$src$apply_shim");
goog.require("module$src$style_util");
goog.require("module$src$common_regex");
goog.require("module$src$common_utils");
goog.require("module$src$css_parse");
var APPLY_NAME_CLEAN$$module$src$apply_shim = /;\s*/m;
var INITIAL_INHERIT$$module$src$apply_shim = /^\s*(initial)|(inherit)\s*$/;
var MIXIN_VAR_SEP$$module$src$apply_shim = "_-_";
var PropertyEntry$$module$src$apply_shim;
var DependantsEntry$$module$src$apply_shim;
var MixinMapEntry$$module$src$apply_shim;
var MixinMap$$module$src$apply_shim = function() {
  this._map = {};
};
MixinMap$$module$src$apply_shim.prototype.set = function(name, props) {
  name = name.trim();
  this._map[name] = {properties:props, dependants:{}};
};
MixinMap$$module$src$apply_shim.prototype.get = function(name) {
  name = name.trim();
  return this._map[name] || null;
};
var invalidCallback$$module$src$apply_shim = null;
var ApplyShim$$module$src$apply_shim = function() {
  this._currentElement = null;
  this._measureElement = null;
  this._map = new MixinMap$$module$src$apply_shim;
};
ApplyShim$$module$src$apply_shim.prototype.detectMixin = function(cssText) {
  return module$src$common_utils.detectMixin(cssText);
};
ApplyShim$$module$src$apply_shim.prototype.transformTemplate = function(template, elementName) {
...
ApplyShim$$module$src$apply_shim.prototype.transformStyle = function(style, elementName) {
  elementName = elementName === undefined ? "" : elementName;
  var ast = module$src$style_util.rulesForStyle(style);
  this.transformRules(ast, elementName);
  style.textContent = module$src$style_util.toCssText(ast);
  return ast;
};
ApplyShim$$module$src$apply_shim.prototype.transformCustomStyle = function(style) {
  var $jscomp$this = this;
  var ast = module$src$style_util.rulesForStyle(style);
  module$src$style_util.forEachRule(ast, function(rule) {
    if (rule["selector"] === ":root") {
      rule["selector"] = "html";
    }
    $jscomp$this.transformRule(rule);
  });
  style.textContent = module$src$style_util.toCssText(ast);
  return ast;
};
ApplyShim$$module$src$apply_shim.prototype.transformRules = function(rules, elementName) {
  var $jscomp$this = this;
  this._currentElement = elementName;
  module$src$style_util.forEachRule(rules, function(r) {
    $jscomp$this.transformRule(r);
  });
  this._currentElement = null;
};
ApplyShim$$module$src$apply_shim.prototype.transformRule = function(rule) {
  rule["cssText"] = this.transformCssText(rule["parsedCssText"]);
  if (rule["selector"] === ":root") {
    rule["selector"] = ":host > *";
  }
};
ApplyShim$$module$src$apply_shim.prototype.transformCssText = function(cssText) {
  var $jscomp$this = this;
  cssText = cssText.replace(module$src$common_regex.VAR_ASSIGN, function(matchText, propertyName, valueProperty, valueMixin) {
    return $jscomp$this._produceCssProperties(matchText, propertyName, valueProperty, valueMixin);
  });
  return this._consumeCssProperties(cssText);
};
ApplyShim$$module$src$apply_shim.prototype._getInitialValueForProperty = function(property) {
...
ApplyShim$$module$src$apply_shim.prototype._consumeCssProperties = function(text) {
  var m = null;
  while (m = module$src$common_regex.MIXIN_MATCH.exec(text)) {
    var matchText = m[0];
    var mixinName = m[1];
    var idx = m.index;
    ...
    module$src$common_regex.MIXIN_MATCH.lastIndex = idx + replacement.length;
  }
  return text;
};
ApplyShim$$module$src$apply_shim.prototype._atApplyToCssProperties = function(mixinName, fallbacks) {
  mixinName = mixinName.replace(APPLY_NAME_CLEAN$$module$src$apply_shim, "");
  var vars = [];
  ...
  if (mixinEntry) {
    ...
    for (p in mixinEntry.properties) {
      ...
      parts = [p, ": var(", mixinName, MIXIN_VAR_SEP$$module$src$apply_shim, p];
      if (f) {
        parts.push(",", f);
      }
      parts.push(")");
...
ApplyShim$$module$src$apply_shim.prototype._replaceInitialOrInherit = function(property, value) {
  var match = INITIAL_INHERIT$$module$src$apply_shim.exec(value);
  if (match) {
    if (match[1]) {
...
ApplyShim$$module$src$apply_shim.prototype._cssTextToMap = function(text) {
...
ApplyShim$$module$src$apply_shim.prototype._invalidateMixinEntry = function(mixinEntry) {
  if (!invalidCallback$$module$src$apply_shim) {
    return;
  }
  for (var elementName in mixinEntry.dependants) {
    if (elementName !== this._currentElement) {
      invalidCallback$$module$src$apply_shim(elementName);
    }
  }
};
ApplyShim$$module$src$apply_shim.prototype._produceCssProperties = function(matchText, propertyName, valueProperty, valueMixin) {
  var $jscomp$this = this;
  if (valueProperty) {
    module$src$style_util.processVariableAndFallback(valueProperty, function(prefix, value) {
      if (value && $jscomp$this._map.get(value)) {
  ...
  for (p in combinedProps) {
    ...
    out.push("" + propertyName + MIXIN_VAR_SEP$$module$src$apply_shim + p + ": " + v);
  }
  if (needToInvalidate) {
...
ApplyShim$$module$src$apply_shim.prototype["detectMixin"] = ApplyShim$$module$src$apply_shim.prototype.detectMixin;
ApplyShim$$module$src$apply_shim.prototype["transformStyle"] = ApplyShim$$module$src$apply_shim.prototype.transformStyle;
ApplyShim$$module$src$apply_shim.prototype["transformCustomStyle"] = ApplyShim$$module$src$apply_shim.prototype.transformCustomStyle;
ApplyShim$$module$src$apply_shim.prototype["transformRules"] = ApplyShim$$module$src$apply_shim.prototype.transformRules;
ApplyShim$$module$src$apply_shim.prototype["transformRule"] = ApplyShim$$module$src$apply_shim.prototype.transformRule;
ApplyShim$$module$src$apply_shim.prototype["transformTemplate"] = ApplyShim$$module$src$apply_shim.prototype.transformTemplate;
ApplyShim$$module$src$apply_shim.prototype["_separator"] = MIXIN_VAR_SEP$$module$src$apply_shim;
Object.defineProperty(ApplyShim$$module$src$apply_shim.prototype, "invalidCallback", {get:function() {
  return invalidCallback$$module$src$apply_shim;
}, set:function(cb) {
  invalidCallback$$module$src$apply_shim = cb;
}});
var $jscompDefaultExport$$module$src$apply_shim = ApplyShim$$module$src$apply_shim;
module$src$apply_shim.default = $jscompDefaultExport$$module$src$apply_shim;
goog.provide("module$src$document_wait");
var readyPromise$$module$src$document_wait = null;
var whenReady$$module$src$document_wait = window["HTMLImports"] && window["HTMLImports"]["whenReady"] || null;
var resolveFn$$module$src$document_wait;
function documentWait$$module$src$document_wait(callback) {
  requestAnimationFrame(function() {
    if (whenReady$$module$src$document_wait) {
      whenReady$$module$src$document_wait(callback);
    } else {
      if (!readyPromise$$module$src$document_wait) {
        readyPromise$$module$src$document_wait = new Promise(function(resolve) {
          resolveFn$$module$src$document_wait = resolve;
        });
        if (document.readyState === "complete") {
          resolveFn$$module$src$document_wait();
        } else {
      ...
      readyPromise$$module$src$document_wait.then(function() {
        callback && callback();
      });
    }
  });
}
module$src$document_wait.default = documentWait$$module$src$document_wait;
goog.provide("module$src$custom_style_interface");
goog.require("module$src$document_wait");
var CustomStyleProvider$$module$src$custom_style_interface;
var SEEN_MARKER$$module$src$custom_style_interface = "__seenByShadyCSS";
var CACHED_STYLE$$module$src$custom_style_interface = "__shadyCSSCachedStyle";
var transformFn$$module$src$custom_style_interface = null;
var validateFn$$module$src$custom_style_interface = null;
var CustomStyleInterface$$module$src$custom_style_interface = function() {
  this["customStyles"] = [];
  this["enqueued"] = false;
};
CustomStyleInterface$$module$src$custom_style_interface.prototype.enqueueDocumentValidation = function() {
  if (this["enqueued"] || !validateFn$$module$src$custom_style_interface) {
    return;
  }
  this["enqueued"] = true;
  module$src$document_wait.default(validateFn$$module$src$custom_style_interface);
};
CustomStyleInterface$$module$src$custom_style_interface.prototype.addCustomStyle = function(style) {
  if (!style[SEEN_MARKER$$module$src$custom_style_interface]) {
    style[SEEN_MARKER$$module$src$custom_style_interface] = true;
    this["customStyles"].push(style);
    this.enqueueDocumentValidation();
  }
};
CustomStyleInterface$$module$src$custom_style_interface.prototype.getStyleForCustomStyle = function(customStyle) {
  if (customStyle[CACHED_STYLE$$module$src$custom_style_interface]) {
    return customStyle[CACHED_STYLE$$module$src$custom_style_interface];
  }
  var style;
...
CustomStyleInterface$$module$src$custom_style_interface.prototype.processStyles = function() {
  ...
  for (var i = 0; i < cs.length; i++) {
    ...
    if (style) {
      var appliedStyle = (style["__appliedElement"]);
      if (appliedStyle) {
        for (var i$0 = 0; i$0 < style.attributes.length; i$0++) {
          var attr = style.attributes[i$0];
          appliedStyle.setAttribute(attr.name, attr.value);
        }
      }
      var styleToTransform = appliedStyle || style;
      if (transformFn$$module$src$custom_style_interface) {
        transformFn$$module$src$custom_style_interface(styleToTransform);
      }
      customStyle[CACHED_STYLE$$module$src$custom_style_interface] = styleToTransform;
    }
  }
  return cs;
};
CustomStyleInterface$$module$src$custom_style_interface.prototype["addCustomStyle"] = CustomStyleInterface$$module$src$custom_style_interface.prototype.addCustomStyle;
CustomStyleInterface$$module$src$custom_style_interface.prototype["getStyleForCustomStyle"] = CustomStyleInterface$$module$src$custom_style_interface.prototype.getStyleForCustomStyle;
CustomStyleInterface$$module$src$custom_style_interface.prototype["processStyles"] = CustomStyleInterface$$module$src$custom_style_interface.prototype.processStyles;
Object.defineProperties(CustomStyleInterface$$module$src$custom_style_interface.prototype, {"transformCallback":{get:function() {
  return transformFn$$module$src$custom_style_interface;
}, set:function(fn) {
  transformFn$$module$src$custom_style_interface = fn;
}}, "validateCallback":{get:function() {
  return validateFn$$module$src$custom_style_interface;
}, set:function(fn) {
  var needsEnqueue = false;
  if (!validateFn$$module$src$custom_style_interface) {
    needsEnqueue = true;
  }
  validateFn$$module$src$custom_style_interface = fn;
  if (needsEnqueue) {
    this.enqueueDocumentValidation();
  }
}}});
var CustomStyleInterfaceInterface$$module$src$custom_style_interface;
module$src$custom_style_interface.CustomStyleProvider;
module$src$custom_style_interface.default = CustomStyleInterface$$module$src$custom_style_interface;
module$src$custom_style_interface.CustomStyleInterfaceInterface;
goog.require("module$src$apply_shim");
goog.require("module$src$template_map");
goog.require("module$src$style_util");
goog.require("module$src$apply_shim_utils");
goog.require("module$src$document_wait");
goog.require("module$src$common_utils");
goog.require("module$src$custom_style_interface");
goog.require("module$src$style_settings");
var applyShim$$module$entrypoints$apply_shim = new module$src$apply_shim.default;
var ApplyShimInterface$$module$entrypoints$apply_shim = function() {
  var $jscomp$this = this;
  this.customStyleInterface = null;
  module$src$document_wait.default(function() {
    $jscomp$this.ensure();
  });
  applyShim$$module$entrypoints$apply_shim["invalidCallback"] = module$src$apply_shim_utils.invalidate;
};
ApplyShimInterface$$module$entrypoints$apply_shim.prototype.ensure = function() {
  var $jscomp$this = this;
  ...
  if (this.customStyleInterface) {
    this.customStyleInterface["transformCallback"] = function(style) {
      applyShim$$module$entrypoints$apply_shim.transformCustomStyle(style);
    };
    this.customStyleInterface["validateCallback"] = function() {
      requestAnimationFrame(function() {
        if ($jscomp$this.customStyleInterface["enqueued"]) {
...
ApplyShimInterface$$module$entrypoints$apply_shim.prototype.prepareTemplate = function(template, elementName) {
  this.ensure();
  module$src$template_map.default[elementName] = template;
  var ast = applyShim$$module$entrypoints$apply_shim.transformTemplate(template, elementName);
  template["_styleAst"] = ast;
};
ApplyShimInterface$$module$entrypoints$apply_shim.prototype.flushCustomStyles = function() {
  this.ensure();
  if (!this.customStyleInterface) {
    return;
  }
...
ApplyShimInterface$$module$entrypoints$apply_shim.prototype.styleSubtree = function(element, properties) {
  this.ensure();
  if (properties) {
    module$src$common_utils.updateNativeProperties(element, properties);
  }
  if (element.shadowRoot) {
    ...
    for (var i$1 = 0; i$1 < children.length; i$1++) {
      this.styleSubtree((children[i$1]));
    }
  }
};
ApplyShimInterface$$module$entrypoints$apply_shim.prototype.styleElement = function(element) {
  this.ensure();
  var $jscomp$destructuring$var0 = module$src$style_util.getIsExtends(element);
  var is = $jscomp$destructuring$var0.is;
  var template = module$src$template_map.default[is];
  if (template && !module$src$apply_shim_utils.templateIsValid(template)) {
    if (!module$src$apply_shim_utils.templateIsValidating(template)) {
    ...
    if (root) {
      ...
      if (style) {
        ...
        style.textContent = module$src$style_util.toCssText(template["_styleAst"]);
      }
    }
  }
};
ApplyShimInterface$$module$entrypoints$apply_shim.prototype.styleDocument = function(properties) {
  this.ensure();
  this.styleSubtree(document.body, properties);
};
if (!window.ShadyCSS || !window.ShadyCSS.ScopingShim) {
  var applyShimInterface = new ApplyShimInterface$$module$entrypoints$apply_shim;
  var CustomStyleInterface = window.ShadyCSS && window.ShadyCSS.CustomStyleInterface;
  window.ShadyCSS = {prepareTemplate:function(template, elementName, elementExtends) {
    ...
    return module$src$common_utils.getComputedStyleValue(element, property);
  }, nativeCss:module$src$style_settings.nativeCssVariables, nativeShadow:module$src$style_settings.nativeShadow};
  if (CustomStyleInterface) {
    window.ShadyCSS.CustomStyleInterface = CustomStyleInterface;
  }
}
window.ShadyCSS.ApplyShim = applyShim$$module$entrypoints$apply_shim;
}).call(this);
...
(function() {
    ...
    window.Polymer._polymerFn = function(info) {
      ...
      try {
        ...
        console.warn("already defined", klass.is);
      }
...
goog.require("module$src$common_utils");
goog.require("module$src$style_settings");
var customStyleInterface$$module$entrypoints$custom_style_interface = new module$src$custom_style_interface.default;
if (!window.ShadyCSS) {
  window.ShadyCSS = {prepareTemplate:function(template, elementName, elementExtends) {
  }, styleSubtree:function(element, properties) {
    customStyleInterface$$module$entrypoints$custom_style_interface.processStyles();
    module$src$common_utils.updateNativeProperties(element, properties);
  }, styleElement:function(element) {
    customStyleInterface$$module$entrypoints$custom_style_interface.processStyles();
  }, styleDocument:function(properties) {
    customStyleInterface$$module$entrypoints$custom_style_interface.processStyles();
    module$src$common_utils.updateNativeProperties(document.body, properties);
  }, getComputedStyleValue:function(element, property) {
    return module$src$common_utils.getComputedStyleValue(element, property);
  }, nativeCss:module$src$style_settings.nativeCssVariables, nativeShadow:module$src$style_settings.nativeShadow};
}
window.ShadyCSS.CustomStyleInterface = customStyleInterface$$module$entrypoints$custom_style_interface;
}).call(this);
  ...
  Polymer.AppLayoutBehavior = [
    ...
    resetLayout: function() {
      ...
      if (Polymer.Async && Polymer.Async.animationFrame) {
        // jason patch: there is a resize loop only in Firefox and only within the popup windows
